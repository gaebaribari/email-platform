...
병원·CRM·타 뉴스레터 툴마다 컬럼명·값 표기가 제각각인 이종(異種) 데이터를, **매핑 정의(YAML)** 로 표준화하고 **3단계 검증**을 거쳐 표준 구독자 모델로 이관하는 설정 기반 ETL. 관리자 화면에선 **"구독자 일괄 추가"** 버튼(모달)으로 노출되며, CSV 업로드 또는 샘플 데이터로 실행한다.
